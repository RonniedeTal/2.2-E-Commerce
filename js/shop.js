...
function calculateTotal() {
  ...
  let reduce = cart.reduce(
    (acc, actual) =>
      acc +
      (actual.id == 1 ? applyPromotionsCart() : actual.price) * actual.quantity,
    0
  );
...
function applyPromotionsCart() {
  ...
  if (promo.id == 1 && promo.quantity >= 3) {
    applyPromo = parseFloat(promo.price - promo.price * 0.2);
  } else {
  ...
  const cartProducts = cart.map((eachProduct) => {
    ...
    cartArr.innerHTML = `
            ...
            <td>${eachProduct.price}</td>
             <td>${eachProduct.quantity}<button onclick="removeFromCart(${
      eachProduct.id
    })">👎 </button></td>
             <td>${
               eachProduct.id === 1 ? applyPromotionsCart() : eachProduct.price
             }</td>`;
    return cartArr;
  });
